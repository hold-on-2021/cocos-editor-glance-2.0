...
const o = Editor.require(`packages://${i}/panel/component/home`),
    r = {
        ...
        "preview-list": Editor.require(`packages://${i}/panel/component/preview-list`),
...
Editor.Panel.extend({
    style: t.readFileSync(Editor.url(`packages://${i}/panel/style/index.css`), "utf-8"),
    template: o.template,
    listeners: {
        "panel-resize"(e) {
            e.target;
            this.vm && (this.vm.width = e.target.clientWidth, this.vm.height = e.target.clientHeight)
        },
        "panel-show"(e) {
            e.target;
            this.vm && (this.vm.width = e.target.clientWidth, this.vm.height = e.target.clientHeight)
    ...
    ready() {
        ...
        e.id = "game", e.style.display = "none", this.appendChild(e), this.vm.width = this.clientWidth, this.vm.height = this.clientHeight
    },
    close() {
    ...
    deleteTheSelectedKeys() {
        ...
        if (e.forEach(e => {
            ...
            }), t = t.map(e => (e.length > 40 && (e = "..." + e.substr(e.length - 37, 37)), e)), 0 !== Editor.Dialog.messageBox({
                type: "question",
                buttons: [Editor.T("timeline.shortcuts.cancel"), Editor.T("timeline.shortcuts.confirm_and_delete")],
                title: "",
    ...
    copyTheSelectedKeys() {
        ...
        this.vm.selected.forEach(t => {
            ...
            i && (e = Math.min(e, t.frame), this._copyCache.push({
                id: t.id,
                path: t.path,
    ...
    pasteTheCopiedKeys() {
        if (!this.vm.clip || !this._copyCache || 0 === this._copyCache.length) return;
        let t = this.vm.frame,
            i = this.vm.clip.id,
            a = this._copyCache.some((e, t) => t > 0 && e.path !== this._copyCache[t - 1].path),
            o = this._copyCache[0];
        ...
        } else this._copyCache.forEach(e => {
            e._path = e.path
        });
        let r = a ? "" : this.vm.node.path,
            n = [];
        if (this._copyCache.forEach((e, a) => {
                s.Clip.queryKey(i, r || e._path, e.component, e.property, e.offset + t) && n.push(e)
            }), n.length) {
            let e = n.map(e => {
                let i = e.path;
                return e.path.length > 35 && (i = "..." + e.path.substr(e.path.length - 32, 32)), `path: ${i}\nproperty: ${e.component?e.component+"."+e.property:e.property} frame: ${e.offset+t}`
            });
            if (e.length > 5 && (e.length = 4, e.push("...")), 0 === Editor.Dialog.messageBox({
                    type: "question",
                    buttons: [Editor.T("timeline.shortcuts.cancel"), Editor.T("timeline.shortcuts.confirm_and_cover")],
                    title: "",
        ...
        this._copyCache.forEach(e => {
            cc.AssetLibrary.loadJson(e.data, (a, o) => {
                let n = o.value;
                s.Clip.addKey(i, r || e._path, e.component, e.property, e.offset + t, n), o.curve && s.Clip.mountCurveToKey(i, r || e._path, e.component, e.property, e.offset + t, o.curve), clearTimeout(h), h = setTimeout(() => {
                    p.emit("clip-data-update")
                }, 400)
            })
        })

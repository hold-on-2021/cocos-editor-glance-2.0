...
e.load = function (P, j) {
    ...
    for (let e in b.hosts) {
        ...
        if (!r.satisfies(n, a, {
                includePrerelease: !0
            })) return delete y[P], j && j(new Error(`Host '${e}' require ver ${a}`)), void 0
    }
    i.series([n => {
        let a = b.packages;
        ...
        i.eachSeries(Object.keys(a), (n, a) => {
            let t = e.find(n);
        ...
        let r = a.join(b._destPath, "i18n", `${m}.js`);
        ...
        if (b.main) {
            ...
            try {
                ...
                return i(new Error(`Failed to load ${b.main}: ${e.stack}`)), void 0
            }
        }
        if (c) {
            let e = new p;
        ...
        if (f && "object" == typeof f)
            for (let e in f) {
                ...
                if (r.icon) {
                    let e = n.nativeImage.createFromPath(a.join(b._destPath, r.icon));
                    o.icon = e
                }
                s.add(i, o)
            }
        ...
        if (b.profiles) try {
            for (let e in b.profiles) {
                let n = l.load(`${e}://${b.name}.json`),
                    a = b.profiles[e];
                Object.keys(a).forEach(e => {
                    void 0 === n.get(e) && n.set(e, a[e])
                })
            }
        } catch (e) {
            d.failed(`Initial Package Profile [${b._destPath}] failed！` + e)
        }
        for (let e in h) {
            let n;
            if (j) n = `${b.name}.${e}`;
            ...
            o.defaults(i, {
                ...
                engineSupport: !1
            }), i.main ? t.existsSync(a.join(b._destPath, i.main)) ? k[n] = i : d.failed(`Failed to load panel "${e}" from "${b.name}", main file "${i.main}" not found.`) : d.failed(`Failed to load panel "${e}" from "${b.name}", "main" field not found.`)
        }
        ...
        if (t._ipc && t._ipc.clear(), t.main) {
            ...
            if (i) {
                ...
                if (a && a.unload) try {
                    ...
                    t.length > 0 && e(n, t)
                })(t._destPath, i.children), delete e[n]
    ...
    for (let n = 0; n < P.length; ++n) {
        let i = P[n];
        if (t.isDirSync(i)) {
            if (-1 !== t.readdirSync(i).indexOf(e)) return a.join(i, e)
        }
    }
    return null
